...
func update_texture_color() -> void:
	match side_id:
		1: Utils.set_child_meshes_material(get_node("factory"), BUILDING_YELLOW_MATERIAL)
		2: Utils.set_child_meshes_material(get_node("factory"), BUILDING_RED_MATERIAL)
		3: Utils.set_child_meshes_material(get_node("factory"), BUILDING_GREEN_MATERIAL)
		4: Utils.set_child_meshes_material(get_node("factory"), BUILDING_BLUE_MATERIAL)

const PAYMENT_PERIOD: float = 0.5
...
func pb_process(delta: float) -> void:
	for robot in Utils.get_robots():
		if is_robot_close_enough(robot):
			side_id = robot.side_id
			update_texture_color()
			break
			
	if side_id != Utils.Side.Neutral:
		process_paying_cycle(delta)

func process_paying_cycle(delta: float) -> void:
...
@onready var capture_point: Node3D = get_node("CapturePoint")
const CAPTURING_DISTANCE: float = 2.0
func is_robot_close_enough(robot: Robot) -> bool:
	return capture_point.global_position.distance_to(robot.global_position)	<= CAPTURING_DISTANCE
